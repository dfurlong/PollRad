...
$aboutProgram = "Perl script PollRad.pl  (Rev 0.2 Oct 13, 2010)   Copywrite (c) 2010 Amperion Inc.";
...
sub CreateLogFileName {
	...
	$logFile = "PollRad\_$year\_$monthStr\_$dayOfMonthStr.log";
...
sub GetValuesSNMP {
	...
	#print ("SNMP BufferLevel (min, max) =  $minString, $maxString;   BufferSetting = $currJitterBufferSetting  [ ", DisplayTime(), " ] \n\n");

        # If the value(s) have changed write message to logfile.
        if($lastMaxJitterValue != $maxJitterValue or $lastMinJitterValue != $minJitterValue){
        	...
        	print ("SNMP BufferLevel (min, max) =  $minString, $maxString;   BufferSetting = $currJitterBufferSetting  [ ", DisplayTime(), " ] \n\n");
                ...
                open LOGFILE, ">> $logFile" or die $!;
		print LOGFILE ("SNMP BufferLevel (min, max) =  $minString, $maxString;   BufferSetting = $currJitterBufferSetting  [ ", DisplayTime(), " ] \n\n");
		close LOGFILE;
        }
...
#  NOTE:
#
#      The following function where used during the development of the program
